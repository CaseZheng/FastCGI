...
typedef struct FCGX_Stream {
    ...
    unsigned char *stopUnget; /* reader: first byte of current buffer
                               * fragment, for ungetc
                               * writer: undefined */
    int isReader;               //用于读的一个缓冲区标记
    int isClosed;               //缓冲区被关闭标记
    int wasFCloseCalled;
    int FCGI_errno;                /* error status */
    void (*fillBuffProc) (struct FCGX_Stream *stream);                  //填充缓冲区的回调函数,用于读缓冲区
    void (*emptyBuffProc) (struct FCGX_Stream *stream, int doClose);    //清空缓冲区的回调函数,用于写缓冲区
    void *data;
} FCGX_Stream;
...
typedef struct FCGX_Request {
    int requestId;              //请求Id valid if isBeginProcessed
    int role;                   //角色
    FCGX_Stream *in;            //输入缓冲区
    FCGX_Stream *out;           //输出缓冲区
    FCGX_Stream *err;           //输出错误缓冲区
	char **envp;

	/* Don't use anything below here */

    struct Params *paramsPtr;
    int ipcFd;                  //监听的文件描述符 小于0表示没有连接
    int isBeginProcessed;       //接收到FCGI_BEGIN_REQUEST后设置的标记
    int keepConnection;         //解决请求后不关闭连接的标记
    int appStatus;
    int nWriters;             /* number of open writers (0..2) */
	int flags;
	int listen_sock;            //监听的socket
} FCGX_Request;

 ...
 * 异步I/O表中为每个文件描述符分配两个
 *
 * Read Entry Index  = fd * 2               读的I/O信息的下标
 * Write Entry Index = (fd * 2) + 1         写的I/O信息的下标
 */
#define AIO_RD_IX(fd) (fd * 2)
#define AIO_WR_IX(fd) ((fd * 2) + 1)

static int asyncIoInUse = FALSE;            //默认不使用异步I/O
static int asyncIoTableSize = 16;           //默认的异步I/O信息表的大小
static AioInfo *asyncIoTable = NULL;        //异步I/O信息表

static int libInitialized = FALSE;          //初始化标记
...
static void installSignalHandler(int signo, const struct sigaction * act, int force)
{
    ...
    sigaction(signo, NULL, &sa);    //设置信号的处理方式为NULL,对信号使用默认的处理方式,并获取信号目前的处理方式到sa

    if (force || sa.sa_handler == SIG_DFL)  //强制设置 或 信号目前的处理方式为默认方式
    {
        sigaction(signo, act, NULL);    //设置信号的处理方式为传入的act
    }
}
...
static int OS_BuildSockAddrUn(const char *bindPath,
                              ...
                              int *servAddrLen)
{
    unsigned bindPathLen = strlen(bindPath);
...
int OS_Close(int fd)
{
    ...
    if (asyncIoInUse) {                 //使用异步I/O,需要做清理工作
        int index = AIO_RD_IX(fd);
...
static int AcquireLock(int sock, int fail_on_intr)
{
    ...
    do {
        struct flock lock;
        lock.l_type = F_WRLCK;  //写入锁(排斥锁)
        lock.l_start = 0;
        lock.l_whence = SEEK_SET;
        lock.l_len = 0;

        //为sock设置lock描述的文件锁 阻塞
...
int OS_Accept(int listen_sock, int fail_on_intr, const char *webServerAddrs)
{
    ...
    for (;;) {
        if (AcquireLock(listen_sock, fail_on_intr)) //给listen_sock加锁
            return -1;
        ...
        for (;;) {
            ...
            } while (socket < 0                 //accept失败
                     && errno == EINTR          //接收到中断信号
                     && ! fail_on_intr          //
                     && ! shutdownPending);     //未设置退出标记

            if (socket < 0) {   //未接收到socket
                if (shutdownPending || ! is_reasonable_accept_errno(errno)) {
                    int errnoSave = errno;

                    ReleaseLock(listen_sock);   //解锁
                ...
                if (sa.in.sin_family != AF_INET)    //不是IPv4
                    break;
        ...
        if (ReleaseLock(listen_sock))   //解锁
            return (-1);

        if (sa.in.sin_family != AF_UNIX || is_af_unix_keeper(socket))   //不是本地间通信 或者 socket可读
            break;
...
int OS_IsFcgi(int sock)
{
    ...
    //获取套接字的对端协议地址 当获取失败且错误码为ENOTCONN时表示FastCGI

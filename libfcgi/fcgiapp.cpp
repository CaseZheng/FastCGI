 ...
 * Globals
 */
static int libInitialized = 0;      //全局变量 初始化标记
...
int FCGX_GetStr(char *str, int n, FCGX_Stream *stream)
{
    ...
    // Fast path: n bytes are already available 判断缓冲区有n个字节可用
 ...
 * MakeHeader --
 *
 *      Constructs an FCGI_Header struct.   构造FCGI_Header
 *
 *----------------------------------------------------------------------
 */
        ...
        int paddingLength)
{
    ...
    header.version = FCGI_VERSION_1;                                            //版本为固定值
    header.type             = (unsigned char) type;                             //操作类型
    header.requestIdB1      = (unsigned char) ((requestId     >> 8) & 0xff);    //请求Id
    header.requestIdB0      = (unsigned char) ((requestId         ) & 0xff);
    header.contentLengthB1  = (unsigned char) ((contentLength >> 8) & 0xff);    //消息体长度
    header.contentLengthB0  = (unsigned char) ((contentLength     ) & 0xff);
    header.paddingLength    = (unsigned char) paddingLength;                    //填充字节长度
    header.reserved         =  0;                                               //保留字节
    return header;
}
...
/*
 * State associated with a stream 缓冲区状态信息
 */
...
static int ProcessBeginRecord(int requestId, FCGX_Stream *stream)
{
    ...
    if(data->reqDataPtr->isBeginProcessed) {
        // Web服务器通过同一个连接,发送两个开始请求的消息,此处不支持,直接回复FCGI_END_REQUEST{protocolStatus = FCGI_CANT_MPX_CONN}
        FCGI_EndRequestRecord endRequestRecord;
    ...
    // Accept this new request.  Read the record body. 接收到一个新的请求,读取请求消息体
    data->reqDataPtr->requestId = requestId;        //记录请求Id
    if(FCGX_GetStr((char *) &body, sizeof(body), stream) != sizeof(body)) { //读取请求开始消息体
        return FCGX_PROTOCOL_ERROR;
    }
    data->reqDataPtr->keepConnection = (body.flags & FCGI_KEEP_CONN);   //保持连接的标记
    data->reqDataPtr->role = (body.roleB1 << 8) + body.roleB0;          //角色
    data->reqDataPtr->isBeginProcessed = TRUE;                          //开始请求的标记
    return BEGIN_RECORD;
}
...
static int ProcessHeader(FCGI_Header header, FCGX_Stream *stream)
{
    FCGX_Stream_Data *data = (FCGX_Stream_Data *)stream->data;
    int requestId;
    if(header.version != FCGI_VERSION_1) {      //判断下FastCGI协议版本
        return FCGX_UNSUPPORTED_VERSION;
    }
    requestId        = (header.requestIdB1 << 8) + header.requestIdB0;         // 获取请求Id
    data->contentLen = (header.contentLengthB1 << 8) + header.contentLengthB0; // 获取消息长度
    data->paddingLen = header.paddingLength;                                   // 获取填充字节长度
    if(header.type == FCGI_BEGIN_REQUEST) {             //处理请求的开始消息
 ...
 *      创建一个用于读取数据的缓冲区给一个请求,这个缓冲区最多存bufflen个字节
 *
 *----------------------------------------------------------------------
 */
 ...
 *      pipe (ie. standard in).
 *      确定进程是作为CGI还是FastCGI运行,通过确定FCGI_LISTENSOCK_FILENO判断
 *
 * Results:
 *      TRUE if the process is a CGI process, FALSE if FastCGI.
 *      返回TRUE是CGI程序 返回FALSE是FastCGI程序
 *
 *----------------------------------------------------------------------
 */
int FCGX_IsCGI(void)
 ...
 *      初始化FCGX库,会被FCGX_Accept调用.用户在调用FCGX_Accept_r()前也需要调用
 *
 * Results:
...
int FCGX_Accept_r(FCGX_Request *reqDataPtr)
{
    ...
    for (;;) {
         ...
         */
        if (reqDataPtr->ipcFd < 0) {    //未连接
            int fail_on_intr = reqDataPtr->flags & FCGI_FAIL_ACCEPT_ON_INTR;

            reqDataPtr->ipcFd = OS_Accept(reqDataPtr->listen_sock, fail_on_intr, webServerAddressList); //阻塞接受连接
        ...
        reqDataPtr->in = NewReader(reqDataPtr, 8192, 0);    //初始化一个输入缓冲区
